				 ...
				 <?php for($i=0;$i<count($chapters);$i++):?> 
					...
					<?php endif;?>
							<div class="accordion-inner">
								...
								<?php for($j=0,$num=1;$j<count($chapters[$i]['lectures']);$j++):?>
										<tr>
											...
											<td><?=anchor('physical/courses/current/course/'.$this->uri->segment(5).'/lecture/'.$chapters[$i]['lectures'][$j]['id'],$chapters[$i]['lectures'][$j]['title']);?><a href="#"></a></td>
										</tr>
										<?php $num++;?>
								<?php endfor;?>
									</tbody>
							...
							<?php if(isset($chapters[$i]['test']['id'])):?>
								<?=anchor($this->uri->uri_string().'/testing/id/'.$chapters[$i]['test']['id'],'Промежуточное тестирование',array('class'=>'btn btn-info'));?>
...
Результат: <?=$chapters[$i]['test']['result'];?>% <?=($chapters[$i]['test']['result'] > 60) ? '<font style="color:#0000ff">(зачет)</font>' :'<font style="color:#ff0000">(незачет)</font>';?>
				...
				<?php endfor;?>
					...
					<?php if($testvalid):?>
						<div class="btn-group">
							<?=anchor($this->uri->uri_string().'/final-testing/id/'.$test['id'],'Итоговое тестирование',array('class'=>'btn btn-warning'));?>
						</div>
					<?php endif;?>
