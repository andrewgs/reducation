...
<body>
			...
			<div class="span9">
				...
				<table class="table table-striped table-bordered">
					<thead>
						<tr>
							<th>&nbsp;</th>
							<th class="centerized"><?=anchor('admin-panel/messages/deleted/orders/id/'.$sortby.'/'.$from,'№ заказа');?><span id="id"></span></th>
							<th class="centerized"><nobr><?=anchor('admin-panel/messages/deleted/orders/paiddate/'.$sortby.'/'.$from,'Заказ создан');?><span id="paiddate"></span></nobr><br/><nobr><?=anchor('admin-panel/messages/deleted/orders/closedate/'.$sortby.'/'.$from,'Заказ закрыт');?><span id="closedate"></span></nobr></th>
							<th class="centerized"><?=anchor('admin-panel/messages/deleted/orders/organization/'.$sortby.'/'.$from,'Заказчик');?><span id="organization"></span></th>
							<th class="centerized">
								<?=anchor('admin-panel/messages/deleted/orders/userpaiddate/'.$sortby.'/'.$from,'Дата оплаты');?><span id="userpaiddate"></span>
								<br/>Дата обучения
							</th>
							<th>&nbsp;</th>
						</tr>
					...
					<?php for($i=0;$i<count($orders);$i++):?>
						<tr>
							...
							<?php if($orders[$i]['finish']):?>
								&nbsp;<?=anchor('admin-panel/messages/orders/id/'.$orders[$i]['id'].'/testing','<img src="'.$baseurl.'img/icon/document-task.png" />',array('title'=>'Итоговые тесты'));?></nobr><br/><br/>
	...
	<script type="text/javascript">
		...
		<?php endif;?>
			...
			$("#ResOrder").click(function(){location.href='<?=$baseurl;?>admin-panel/messages/orders/restore-order/'+Order;});
		});
	</script>
</body>
</html>
