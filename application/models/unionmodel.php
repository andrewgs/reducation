...
class Unionmodel extends CI_Model{
	...
	function read_corder_records($order){
		...
		$query = "SELECT courseorder.*,courses.title,courses.price AS manual_price,courses.code,orders.discount,orders.docnumber FROM courseorder INNER JOIN courses ON courseorder.course=courses.id INNER JOIN orders ON orders.id = courseorder.order WHERE courseorder.order = $order ORDER BY courseorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
	...
	function read_corder_group_records($order){
		...
		$query = "SELECT courseorder.*,courses.title,courses.hours,courses.price AS manual_price,courses.code, count(audienceorder.id) AS cnt,orders.discount, orders.docnumber FROM courseorder INNER JOIN courses ON courseorder.course=courses.id INNER JOIN audienceorder ON courseorder.id = audienceorder.course INNER JOIN orders ON orders.id = courseorder.order WHERE courseorder.order = $order GROUP BY courseorder.course ORDER BY courseorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
	...
	function read_fullinfo_audience($order){
		...
		$query = "SELECT audienceorder.*,audience.lastname,audience.name,audience.middlename,audience.fiodat,audience.specialty,audience.position,customers.organization,customers.inn,customers.kpp,customers.uraddress,orders.id AS ordid,orders.orderdate,orders.userpaiddate,orders.price AS ordprice,orders.paid,orders.paiddate,orders.discount,orders.docnumber,courses.code AS ccode,courses.title AS ctitle,courses.hours AS chours,courseorder.price AS сprice,courses.price AS manual_price FROM audienceorder INNER JOIN audience ON audienceorder.audience=audience.id INNER JOIN courseorder ON audienceorder.course=courseorder.id INNER JOIN customers ON audienceorder.customer = customers.id INNER JOIN orders ON audienceorder.order = orders.id, courses WHERE audienceorder.order = $order AND courseorder.course = courses.id ORDER BY audienceorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
	...
	function read_course_metodical($audience,$course,$status){
		
		$query = "SELECT courses.metodical FROM courseorder INNER JOIN courses ON courses.id=courseorder.course INNER JOIN audienceorder ON audienceorder.course = courseorder.id INNER JOIN orders ON courseorder.order=orders.id WHERE audienceorder.audience = $audience AND audienceorder.id = $course AND audienceorder.status = $status AND orders.paid = 1";
		$query = $this->db->query($query);
		$data = $query->result_array();
	...
	function read_customer_info_order($order){
		...
		$query = "SELECT number,year,organization,personemail,userpaiddate,orderdate,price,discount,docnumber,closedate FROM orders INNER JOIN customers ON orders.customer = customers.id WHERE orders.id = $order";
		$query = $this->db->query($query);
		$data = $query->result_array();
